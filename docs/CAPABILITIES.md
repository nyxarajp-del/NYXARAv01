...
| L-CHRONOS | `nyxara.nyx.chronos` with `nyxara.mind.mcts_reasoner` | Monte Carlo rollouts over an approximate world model, ranked with tail risk, folded into the superposition as evidence — so collapse favours the option that survived most futures | "Thousands of timelines" means exactly the branch budget, run sequentially. Below a coverage floor it reports itself **blind** rather than emitting zeros as foresight. That floor used to be unreachable: chronos simulated over a world model it built **for itself**, which is born empty and which nothing in the think-cycle ever teaches, so every pass reported `blind` for the life of the process — honest, and permanently so. It now adopts the **kernel's** model (the one the abyss engines already share, filled by the embodied loop's lived transitions) in `attach_kernel`, so coverage can actually rise and the floor can actually be crossed. Two more knobs that were described and inert now bite: `chronos_budget_ms` is the simulator's real wall-clock cap — split evenly across the options so a clipped pass still ranks them on comparable evidence, and reported as `clipped` with the branch count she *actually* rolled rather than the one she asked for — and the rollouts carry a model-error term, without which a pass over one committed option collapsed to a single trajectory repeated N times: identical `p05`/`p95`, a zero-width interval, and a "tail risk" that was just the mean because there was no tail |
...
| What is bound to what | `nyxara.nyx.hyper_vector` over `nyxara.cognition.hyper_dimensional_vectors` | `semantics` answers *how close* two concepts are. It cannot answer **which role is bound to which filler** — and "the engine turns the flywheel" versus "the flywheel turns the engine" is exactly the case where that difference is the entire meaning. A bag of words cannot tell them apart; a bound hypervector can. `encode()` binds each `role → filler` and bundles them into one 10,000-D vector, `role_of()` unbinds one back out and snaps it to a clean symbol, `permute` makes **order structural** (measured: *A then B* vs *B then A* similarity **0.004**, same order **1.0**). **Zero-shot analogy** over structures: given `france = {capital⊗paris, …}` and `japan = {capital⊗tokyo, …}`, `france : japan :: paris : ?` returns **tokyo** — she was never told those two correspond, and nothing was trained. Associative recall over a taught relation is a *separate* method with its own words, because it is a different claim | **Capacity is real and is measured, not quoted.** `capacity_probe()` finds where cleanup actually breaks on this machine: at dim 256 she retrieves **15 bindings cleanly and fails at 16**; at dim 10,000 she is still clean at 24. Every retrieval carries a **margin**, and one that barely beats its runner-up is reported **untrusted** rather than returned as fact — this is *measured retrieval*, never "absolute retrieval". Over **bare atomic symbols there is no relation to transport** (two random vectors share no structure) and she says so instead of dressing the nearest neighbour up as an inference. An analogy over a vocabulary she was never given returns **nothing**. Nothing here is trained — which is what makes the analogy zero-shot, and also its ceiling: the structure has to be given to her |
| A claim that carries its proof | `nyxara.nyx.theorem_prover` with `z3`, `sympy`; feeds `nyxara.nyx.hybrid` | The control law was already *verifiable beats probabilistic* — but "verifiable" meant **her own engines agreed**. This gives the word a machine-checkable meaning where the claim admits one. `for all real x, x*x >= 0` → **PROVED** with the certificate *¬F is unsatisfiable*; `2 + 2 = 5` → **REFUTED**; `for all real x, x*x >= 1` → **REFUTED with the counter-model x=0**, because the quantifier decides what a counter-model *means*; `there exists real x, x*x = 4` → **PROVED with the witness x=2**; `x > 5` → **CONTINGENT**, named as such. SymPy is the second engine behind z3 for exact algebra. `hybrid` now upgrades a formally expressible claim to a machine-checked verdict, and leaves everything else on the unchanged evidence path | **"Eliminate hallucination mathematically" cannot be fully true and is not claimed anywhere.** A proof runs only on **formally expressible** claims — *"gravity pulls the apple down"* is not an SMT formula, and asked to prove it the module returns **INEXPRESSIBLE and no verdict at all**. That refusal is the feature; it is also the most-tested behaviour in the file. A bare term (`gravity`, `x + 1`) has no truth value and is refused for the same reason. A timeout or `unknown` is reported as *"I could not prove this"*, **never** as *"it is correct"*. **Contingent is not false** — `x > 5` is neither, and calling it refuted would be a lie about a claim that is simply about a free variable; `certify()` returns nothing for it, so it never becomes evidence. The claim language is closed and small, by construction rather than by judgement |
| "Because", not "alongside" | `nyxara.nyx.causal_engine` over `nyxara.mind.causal_world_model` | Her concept graph is built on Hebbian co-activation, and **co-activation is symmetric**: every edge in it says *these two turn up together* and not one of them says *this one brings the other about*. The Pearl machinery was already in the repo — `do()`, back-door and front-door adjustment, confounder search, counterfactuals — and nothing in `think()` had ever called it. `CausalReasoner` answers a *why* or a *what-if* turn by **intervening**: measured here, rain observed before wet across a real timeline is identified **causal at 0.78 confidence with `do(rain) = +0.58`**, while two things that merely alternate come back *not established as causal*. Every answer prints the intervention **beside** the co-occurrence weight, with its identification strategy, its confounder and its confidence — and `CausalSpecialist` bids on the workspace stage **only** when the effect was actually identified | **Identification rests on the graph and on the assumptions behind it.** Given the wrong graph, do-calculus returns a **confident wrong answer** — that is a property of the method, not a bug here, so every verdict carries the strategy, the confounder and the confidence and none of the three is optional. A causal edge is **never** derived from co-occurrence alone: it needs a verdict from the world model, and it is stored in a **separate directed store** so "turned up together" and "brought about" can never be read as the same number — laying one does **not** strengthen the Hebbian weight underneath. A pair with too little evidence is reported as too little evidence, not as *unrelated* |
| A graph that can say what kind of edge | `nyxara.nyx.meta_architecture` over `nyxara.nyx.graph` | V.01's graph has exactly **one** edge kind — a co-occurrence weight — so *is-a*, *part-of*, *causes* and *contradicts* all had to be the same number. `GraphWeaver` layers typed, directed edges over it without touching a single Hebbian value: seven builtin relations, edges read out of sentences that actually state one (*"a sparrow is a bird"*, *"friction causes heat"*), and **node splitting** for a polysemous label — *bank* with a `loan/mortgage/interest` neighbourhood and a `river/sediment/erosion` neighbourhood becomes `bank#1` and `bank#2`, each keeping its own half of the structure | **"Rewires itself like a biological brain every second" is an analogy**, and is written down as one — this is real structural plasticity **on a budget**, not synaptogenesis. A **new edge type needs support, not an idea**: below `min_support` a pattern is a counted candidate and nothing is minted, and the type table itself is capped. A node is split **only when its two neighbourhoods are genuinely disjoint**, because splitting a node that was not polysemous destroys real structure to fix an imagined problem. Every schema change goes to a ledger and `undo_last()` reverses it — a minted type takes its edges with it. The per-tick rewire budget is spent and then she stops |
| An answer that has been attacked before you see it | `nyxara.nyx.dialectic` with `nyxara.mind.inner_critic`, `nyxara.nyx.theorem_prover` | `workspace` already runs a competition — but it is a competition for **salience**, and the winner is the loudest coalition rather than the one that survived being torn apart. Nothing in NYX had ever attacked a conclusion **after** it won and **before** it shipped. Proponent versus Skeptic, with real weapons: `InnerCritic` for vagueness and contradiction, a universal-with-no-evidence check, a missing-mechanism check, and the sharpest one there is — *prove it* — which can **refute** an answer outright. Three outcomes, all real: it survives; it survives with its **confidence cut**; or she **abstains**, and the answer reaching the Master is empty. The debate rides on the thought and into `/nyx why` | **This is not "bulletproof"** and the word is refused in `stats()`. It is a *structured critique* with a **finite** objection set — vagueness, contradiction, unsupported assertion, missing mechanism, formal refutation — and an error outside that set walks straight through. It is **budgeted, and the budget is enforced rather than declared** — `budget_ms` was stored and never consulted, so the one expensive weapon could overrun a turn's budget by an order of magnitude on exactly the claim hardest to decide. The prover is now handed *the time that is actually left* instead of its own 3000 ms worst case; a proof that fits still happens, one that does not returns "could not prove", which this module already treats as no objection. When the budget runs out mid-attack she stops rather than overruns, and the result carries `truncated`, because **surviving a shortened attack is not the same claim** as surviving a full one. A turn that was not debated at all records `debated=False`, so the difference between *attacked and survived* and *never attacked* is never invisible. **Confidence going down after a debate is a success**, not a failure: the point is not to win the argument, it is to know how much of the answer was load-bearing |
| What happened while you were away | `nyxara.nyx.stream` with `nyxara.nyx.aura`, `nyxara.nyx.car` | `aura` already takes events in continuously and `car` already thinks a self-directed thought between prompts. What neither does is **synthesise**: a thousand events arrive, are absorbed one at a time, and nothing ever compresses them into something a person could read on returning. Four scales at once — seconds, minutes, hours, days — each with its own window and its own summary, because *what just happened* and *what has been happening all week* are different questions and one buffer cannot answer both. A digest keeps what was **new**, what **repeated** and what **changed**, not the raw stream; her own agenda findings and omega promotions are absorbed as things that happened; and `briefing()` is one paragraph for someone who has just come back | **"Never sleeping" is true only while the process is running.** `nyxara-daemon` runs; a CLI session that exits does not — the tick count keeps the claim checkable, and `/scram` stops it dead. It rides the **existing** heartbeat, with no second thread, which is the same rule V.01 kept. It digests **what actually arrived** — registered streams, her own turns, her own beats — and never reaches out on its own; that is `aura`'s job and its own network policy. A digest reports what she **saw**, not what it means, and a scale that has seen nothing says so rather than producing an empty-sounding summary. The raw stream is deliberately **not** persisted: compressing it is the whole point |
| One command, decomposed and actually run | `nyxara.nyx.sovereign_intent` with `nyxara.nyx.intent`, `nyxara.nyx.consequence`, `nyxara.nyx.hands`, `nyxara.nyx.author`, `nyxara.nyx.theorem_prover`, `nyxara.nyx.dialectic`, `nyxara.nyx.causal_engine` | `agenda` pursues goals **she** chose; `hands` runs one tool; `author` writes one function. The thing between them never existed: one sentence from the Master becoming a plan with parts, an order, a risk class per part and a way back per part. `GoalTree` reads the DAG out of the instruction itself — `intent`'s actions, its `(before, after)` ordering, its constraints, and a sequencer split so *"write a doubler **and then** verify it"* is two nodes with an edge, while *"fix the code **but first** run the tests"* wires an edge that runs **backwards through the text**. Every node is priced by `ConsequenceGate` at **plan** time — reversibility, blast radius, tail risk — and given its rollback strategy (git branch, workspace snapshot, omega rollback point) **before** the first node executes. A failure asks `causal_engine` for a root cause and inserts a repair node rather than stopping, and the whole tree persists so `resume()` picks up where it stopped | **Her reach is exactly `author`'s bounded synthesis and the registry's tools** — nothing here widens either. Ask for a 206k-line C++ port and what you get is a real DAG, real attempts on the nodes she can reach, real measurements, and an honest report of where it stopped: a node she cannot complete is marked with its reason and the plan reports itself **INCOMPLETE**, never *done*. **Silently shipping broken code is the one thing this repo does not do.** Decomposition is **structural, not semantic** — it splits on the orderings and conjunctions `intent` actually found, so a one-clause instruction is a one-node tree and says so rather than being padded out. A node that needed nothing is counted as **neither** a success nor a failure, and a plan where every node had nothing to show says that calling it done would be the wrong word. Every node acts through the registry and the consequence gate, so permissions, capability bounds, the audit log and `/scram` all apply exactly as before — nothing here is a side door |
...
| L-OMEGA | `nyxara.nyx.omega` with `nyxara.nyx5.autopoiesis`, `nyxara.growth.rule_synth`, `nyxara.growth.lineage` | L-OMNI rewrites her **code**; `author` writes **new** code. Neither touches the constants her graph and memory actually run on — `hebbian_rate`, `decay_rate`, `recall_threshold` were numbers somebody typed once and never measured against an outcome. This hill-climbs them with a **bandit over which knob is worth trying**, against a fitness read from meta-cognition's *measured* correctness and from real recall quality. Both `DynamicNeuralGraph` and `HoloMemory` grow a `knobs()`/`apply_knobs()` seam with a declared whitelist and per-knob ranges. Every change goes through `AutopoieticRewriter` — apply, gauntlet, and **roll back anything that did not beat its own baseline** — and every promotion and rollback is a signed lineage entry. When the fitness *stalls*, she stops turning knobs and asks `rule_synth` for a **learning rule that is not in the menu at all**: the part that is not tuning | **She tunes her knobs, not her constitution.** The tunable set is a whitelist, and the safety core is refused **twice over** — here by name and again inside `AutopoieticRewriter`. Capacity is deliberately not a knob: growing it is a resource decision. **Not "better every second"** — one change at a time on a budgeted cadence, because a mind that reshapes itself every tick has no baseline left to measure against. **Below a sample floor she does not evolve at all**, and `force` does **not** waive that floor — it waives the *cadence*, because "run now" and "hill-climb on noise" are different requests; waiving the floor needs its own `ignore_sample_floor`, after which the step is marked `measured: false` so its score is never read back as evidence. **A probe with nothing to read abstains**: an empty graph, a graph too small for its mean degree to mean anything, and an empty store each contribute nothing rather than a free win. **The one-step gauntlet can only see a knob that moves retrieval** — `recall_threshold` moves it now, `hebbian_rate` and its neighbours act over later turns; those are reported as *not measurable in one step*, never as having failed, because no evidence was gathered either way, and the bandit counts them so it stops picking them. It is **greedy** hill-climbing, so it cannot cross a flat region: a knob parked far from the gradient stays there, and that is stated rather than hidden. Every step is reversible and `rollback()` restores the knobs *exactly*. Without the gauntlet nothing is applied. `/scram` stops her |
| L-AXIOM-GENESIS | `nyxara.nyx.axiom` with `z3`, `nyxara.nyx.nexus` | Every reasoning engine in this repo starts from premises somebody gave it. This does the thing a mathematician does when no existing system fits: **writes a new set of axioms down and checks whether it holds together**. A description becomes properties of an uninterpreted binary relation (matched on word boundaries, so "irreflexive" does not also mean "reflexive"); the quantifiers are expanded over a finite domain so the check is **decidable and its bound is visible**; z3 searches for a model. Independence is checked the only honest way available — is there a model of *the rest* in which this axiom fails? — because an axiom that follows from the others is not a new axiom. Theorems are derived inside the system, `nexus` mints glyphs of her own for it, and promotion needs **both** a model and an independent axiom. Real result, measured here: `{transitive, cyclic-pair}` has a model at size 2 and is promoted; `{irreflexive, transitive, cyclic-pair}` has none | She invents **new formal systems, not new physical truths** — a consistent system is *coherent*, not *true*. **Gödel is not evaded**: a model found proves consistency **at that size**; no model found proves *nothing*, and is reported as "no model up to size N — that is not a proof of inconsistency, and not a proof of consistency either". She will **not** prove Riemann; what is real is the axiom-invention step itself. The axiom language is **closed** and stated as closed — a description outside it is refused with the whole list. Search is budgeted by a domain bound and a per-call timeout: there is no infinite speed. Without z3 the layer reports itself unavailable rather than guessing, because an axiom system nobody checked is not a result. The headline result above only became true when a real gap was closed: `cyclic-pair` was expressible, listed in `PROPERTIES`, and **reachable by no phrasing at all** — the cues covered "cycle" and "both ways", and `\bcycle` does not match "cyclic" — so *"an irreflexive transitive relation with a cyclic pair"* quietly built the weaker `{irreflexive, transitive}`, found a model for **that**, and promoted it. She answered a question nobody asked and called it a success. Every property now answers to its own name, derived from `PROPERTIES` so the invariant holds for any property added later, and the render's row label no longer reads `consistent:` above a value that says no model was found |
| L-CHRONO-CAUSAL | `nyxara.nyx.consequence` with `nyxara.nyx.chronos`, `nyxara.abyss.timeline_simulator`, `nyxara.agency.git_tool` | L-CHRONOS simulates futures for decisions **inside** `think()` — which answer to give. It had never run on a **tool call** or on **code being written**, which was survivable while her brain was blind to tools; with `hands` and `author` aboard it is the gap that matters. This gate sits **in front of execution**: a model of the action from checkable facts (which paths, inside the workspace or not; what `git diff --stat` actually reports; the tool's own registered `risk`/`reversible`), a **reversibility class** derived from those — `undoable` / `recoverable` / `irreversible` — bounded rollouts ranked by **CVaR** rather than the mean, and her own restraint on a bad tail. Where a genuinely reversible route to the same intent exists she takes it herself: committing onto `main` becomes *branch first, then commit*. This is **her foresight, not an escalation** — nobody is asked for permission, so full autonomy is untouched | "Ten thousand timelines" means exactly the branch budget, run sequentially, and it is reported as a number. **Five years is not simulable** — no world model here carries five years of dynamics, and the horizon covers her own system state (which files change, what git does, what the blast radius is), not the world. There is **no "100% optimal"**: she reports tail risk and confidence, and calls herself **blind** rather than emitting zeros as foresight. One rule is not advisory: **cannot see ahead + cannot be taken back ⇒ she does not act**, and a gate that fails internally fails *closed*. No "safe variant" is ever invented for an action that has none — that looks like care and is not, so the search returns nothing far more often than something. `blast_radius` counts paths, not outbound calls, because a field whose job is to be a right number must be one. And **unmodelled is not the same as safe**: the reversibility class started at `undoable` and was only ever made worse by a modelled effect, so an action she could model *nothing* about — free text with no args produces no path, no registry entry, no outbound token — came out `undoable` and **allowed**. `rm -rf /some/unknown/path` was measured passing that way. An action with no modelled effects is now `unknown`, which `_decide` already treats as un-take-back-able, and it is refused **in its own words** rather than borrowed ones. A *registered* tool always contributes an effect, so a read-only tool that touches nothing still reads as undoable — the distinction is *modelled nothing*, not *does nothing*. A plan node is a clause and not yet a tool call, so `GoalTree` prices it advisory-only and the binding check still happens at execution, when `hands` asks this same gate with the real tool and its real arguments |

### Reachable over the wire

A faculty that is not reachable over HTTP is, for a daemon-shaped process, not reachable at all.
All nineteen are on the API surface, and the surface keeps the same rule the modules do.

| Route | What it does | The rule it keeps |
|---|---|---|
| `POST /v1/nyx/{intent,reason,learn,prove,debate,causal,axiom}` | Read a request's structure; work down the reasoning tiers; induce a procedure from demonstrations; a machine-checked verdict; the Skeptic's attack; `do(X)` beside the co-occurrence weight; a new axiom system, checked | Pure faculties — none of them touches the world. Each reply carries the module's own honest limit: `prove` returns **INEXPRESSIBLE** for a claim that is not a formula, `causal` says *not established* rather than returning association wearing the word cause, `reason` names the tiers it tried when every one declines |
| `POST /v1/nyx/{author,evolve}` | Write code through the gauntlet; tune one constant she thinks with | Both are already gauntleted and reversible in the module. `author` returns **every stage**, so the caller sees where it stopped rather than only that it did; `evolve` returns the step *and* the ledger behind it |
| `POST /v1/nyx/{act,goal}` | Reach for a tool; decompose a command into a priced DAG | The only two that can touch the world, and both **default to saying what she would do**: `act` is `dry_run=true` and `goal` is `run=false` unless the caller asks otherwise. A dry run does not move the tool's track record, because crediting a use for something that never ran would teach her a reliability she has not measured. Execution goes through `ToolRegistry.invoke` and the consequence gate exactly as on the console — there is no path here that bypasses either, and `/v1/chat` is still the only way a *turn* can act |
| `GET /v1/nyx/{agenda,stream/digest}` | Her own goals and what she is stuck on; what arrived while you were away, at four scales | Read-only. Registered **before** the `{faculty}` catch-all, because FastAPI matches in registration order and a literal path declared after it is dead — which is a bug that looks exactly like a working route |
| `GET /v1/nyx/<faculty>` | Read-only state for any faculty, V.01 or V.02 | Returns that faculty's `stats()`, which by this repo's honesty rule carries its **limit** and not only its numbers. An unknown name is refused **by name** rather than guessed at |
| `WS /v1/nyx/telepathy` | Meaning as structure, both ways — send a frame, or ask her to emit one | **No mind reading, and none claimed.** What is real is that past the frame nothing is lost. Authenticated before the socket is accepted, like `/v1/ws` |

Every route degrades rather than raises: a faculty switched off answers with `available: false` and
the environment variable that switched it off, so a disabled deployment is diagnosable from the
reply instead of from a stack trace.
