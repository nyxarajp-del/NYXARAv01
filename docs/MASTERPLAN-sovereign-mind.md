...
- **Local teacher available** (the in-process Qwen2.5-0.5B base) → distillation can use a real
...
- **A1 · Distillation — turn the teacher into a teacher (the bridge).**
  Set `NYXARA_FOUNDRY__BACKEND=lora`, `NYXARA_FOUNDRY__BASE_MODEL=Qwen/Qwen2.5-0.5B-Instruct`. The base
...
- **A4 · Base-model ladder + retrieval-augmented own-model.**
  Climb as compute allows: Qwen2.5-0.5B LoRA (today's base) (`FoundryConfig` already has profiles +
