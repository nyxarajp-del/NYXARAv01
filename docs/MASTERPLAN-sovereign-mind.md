...
- **Local teacher available** (the in-process DistilGPT-2 base) → distillation can use a real
...
- **A1 · Distillation — turn the teacher into a teacher (the bridge).**
  Set `NYXARA_FOUNDRY__BACKEND=lora`, `NYXARA_FOUNDRY__BASE_MODEL=distilgpt2`. The base
...
- **A4 · Base-model ladder + retrieval-augmented own-model.**
  Climb as compute allows: DistilGPT-2 LoRA (today's base) (`FoundryConfig` already has profiles +
...
**Next frontier:** A1 (distillation corpus → `foundry.collect_corpus()`) + LoRA-on-DistilGPT-2 config + a
...
beats frontier Claude/GPT, and this box has no GPU/torch — so the neural LoRA-on-DistilGPT-2 brain remains
...
Master's GPU — the LoRA-on-DistilGPT-2 self-model that lifts the *handoff rate* — is scaffolded and one command
away (`python -m nyxara.growth --distilgpt2 --distill --bench`).
