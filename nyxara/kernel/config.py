...
class AgencyConfig(BaseModel):
    ...
    # yet ON by default (the Master's standing choice) so a fresh NYXARA reaches the whole disk,
    # root included, from first boot. It elevates *with* the Master's own credential — never an
    # exploit, guess or brute-force — and is deliberately NOT part of full_control's
    # _OPERATIONAL_CAPS, so turning full_control on never confers root; only this flag does.
    # /scram + oversight + corrigibility, the refusal of UNTRUSTED authority (Rule 7) and the
    # owner-exclusive caps (Rule 8) all remain intact. Set
    # NYXARA_AGENCY__PRIVILEGE_ESCALATION=false to disable and fall back to per-action escalation.
