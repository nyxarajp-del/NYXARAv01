...
class CreativeEngine:
    ...
    def _offline_generate(self, prompt: str) -> str:
        """Real ideation with no LLM: run her own structured creativity engine."""
        ideas = self.select(self.brainstorm(prompt, techniques=list(Technique)), k=4)
        if not ideas:
            return f"(no idea generated for: {prompt.strip()})"
        head = f"Ideas for «{prompt.strip()}» — generated by structured creativity:"
        body = "\n".join(f"{n}. [{i.technique}] {i.content}" for n, i in enumerate(ideas, 1))
        return f"{head}\n{body}"
    ...
    def write(self, brief: str, *, form: str = "prose") -> str:
        ...
        return self._offline_write(brief, form)

    def _offline_write(self, brief: str, form: str) -> str:
        """Compose a real, themed piece offline from her own divergent ideas."""
        f = (form or "prose").lower()
        if "haiku" in f or "poem" in f or "verse" in f:
            return self._compose_verse(brief, form)
        ideas = self.select(
            self.brainstorm(brief, techniques=(Technique.ANALOGY, Technique.LATERAL,
                                               Technique.SCAMPER)), k=3)
        title = brief.strip().rstrip(".") or "untitled"
        opening = (f"On {title} — a short {f}, composed offline from structured "
                   f"creative angles:")
        points = [f"• {i.content}" for i in ideas] or [f"• {brief.strip()}"]
        closing = "Woven together, these angles point to a piece worth developing."
        return f"# {title.capitalize()} ({f})\n\n{opening}\n\n" + "\n".join(points) + \
               f"\n\n{closing}"

    def _compose_verse(self, brief: str, form: str) -> str:
        """A short, themed verse built from the brief's key words and a metaphor domain."""
        kws = _words(brief) or ["the path"]
        domain, principle = self.rng.choice(list(_DOMAINS.items()))
        domain_display = domain.replace("_", " ")
        subject = kws[0]
        echo = kws[1] if len(kws) > 1 else "the quiet"
        first_principle = principle.split(";")[0].strip()
        label = form.strip() or "verse"
        return (f"[{label}] {brief.strip()}\n"
                f"{subject.capitalize()}, like the {domain_display} —\n"
                f"{first_principle},\n"
                f"{echo} endures.")
    ...
    def code(self, spec: str, *, language: str = "python") -> str:
        ...
        return self._offline_code(spec, language)

    def _offline_code(self, spec: str, language: str) -> str:
        """Offline code-gen: real working Python via the Capability Foundry's recipes;
        a clearly-labelled scaffold only when the spec matches no real recipe."""
        name = re.sub(r"[^a-z0-9]+", "_", spec.lower()).strip("_")[:40] or "task"
        if language.lower() in ("python", "py"):
            real = self._foundry_source(spec, name)
            if real is not None:
                return real
        # honest scaffold — the spec matched no known recipe (and there is no LLM)
        return (f'def {name}(*args, **kwargs):\n'
                f'    """{spec}\n\n    Offline scaffold — no matching recipe; refine with a '
                f'real implementation.\n    """\n'
                f'    raise NotImplementedError({spec!r})\n')

    @staticmethod
    def _foundry_source(spec: str, name: str) -> Optional[str]:
        """Real, tested handler source from the foundry's recipe library, renamed + documented."""
        try:
            from nyxara.growth.capability_foundry import best_recipe
            recipe = best_recipe(spec)
        except Exception:  # noqa: BLE001 — the foundry is a bonus, never a hard dependency
            return None
        if recipe is None or recipe.key == "generic":
            return None
        src = recipe.source.replace("def handle(", f"def {name}(", 1)
        # insert a docstring just after the renamed signature line
        out: List[str] = []
        inserted = False
        for line in src.splitlines(keepends=True):
            out.append(line)
            if not inserted and line.lstrip().startswith(f"def {name}("):
                out.append(f'    """{spec.strip()} — {recipe.description}."""\n')
                inserted = True
        return "".join(out)
