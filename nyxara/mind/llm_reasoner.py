...
class LLMReasoner:
    ...
    def flush_online_learning(self, *, budget: Optional[int] = None) -> Any:
        ...
        The turn/tick-driven counterpart to :meth:`teach_self_brain` (which only *queues*): it makes
        the weight update actually happen every turn — and every autonomic tick when idle — instead of
        only when she generates a reply. Returns the brain's :class:`LiveLearnReport`, or None when no
        own brain has been built (a real external LLM is in use). Best-effort; never raises."""
        step = getattr(self._self_brain, "online_step", None)
