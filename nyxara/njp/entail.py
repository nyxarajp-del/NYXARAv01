...
__all__ = ["Pair", "Reasoner", "CORPUS", "LABELS", "read_pairs", "probe"]

#: Where ``scripts/build_reasoning_corpus.py`` writes and this reads.
CORPUS = Path(__file__).with_name("data") / "flan_cot.jsonl.gz"
...
#  measuring a pair
# --------------------------------------------------------------------------------------------- #
def probe(premise: str, hypothesis: str) -> Dict[str, Any]:
...
class Reasoner:
    ...
    def __init__(self, *, min_support: int = 20, min_share: float = 0.05, max_rules: int = 6,
                 max_terms: int = 3, learn: bool = True, purity: float = 0.72) -> None:
    ...
    def learn_from(self, pairs: Sequence[Pair]) -> List[Rule]:
        ...
        readings = [(probe(p.premise, p.hypothesis), p.label) for p in pairs]
    ...
    def answer(self, premise: str, hypothesis: str) -> Tuple[str, str]:
        ...
        reading = probe(premise, hypothesis)
