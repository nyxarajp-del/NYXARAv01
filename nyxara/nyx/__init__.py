...
__all__ = [
    ...
    # ``ascent.Verdict`` is aliased (like ConsequenceVerdict / ProofVerdict above) because the
    # bare name is already bound to ``hybrid.Verdict``: imported last, it silently shadowed the
    # fusion verdict and made it unreachable through this facade, while "Verdict" appeared twice
    # in __all__ so ``import *`` exported only the survivor.
