...
#   * Foundry (LoRA) — peft, so NYXARA can LoRA-tune TinyLlama-1.1B as her OWN brain
#
# Usage:
...
if [[ "$WANT_LLM" == "1" ]]; then
  ...
  echo "==> Foundry (LoRA): installing peft so NYXARA can LoRA-tune TinyLlama-1.1B as her OWN brain"
...
echo "  1. (Optional) edit .env — every TinyLlama/QLoRA knob lives there"
echo "  2. Load it:   set -a; source .env; set +a"
echo "  3. Run:       python -m nyxara"
echo
echo "The TinyLlama base + her own forged brain run together as a council (NYXARA asks both, then decides)."
