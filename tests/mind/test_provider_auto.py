"""The ``auto`` provider ladder (mind/llm.py): self→gguf→tinyllama→mock.
...
def test_auto_with_nothing_promoted_falls_to_mock(tmp_path):
    ...
    assert llm.chosen_provider().name == "mock"     # no self model, no heavy deps
...
def test_auto_gates_a_from_scratch_backend_until_opt_in(tmp_path):
    ...
    llm = LLM(settings=s)
    # promoted ngram exists but must NOT auto-serve (it would degrade a pretrained rung)
...
def test_auto_falls_to_next_rung_when_self_raises(tmp_path):
    ...
    from nyxara.mind.llm import MockProvider
    llm = LLM(settings=s, providers={"self": _BoomSelf(s), "mock": MockProvider(s)})
    resp = llm.complete(LLMRequest.from_prompt("resilient?"))
    assert resp.provider == "mock"                   # honest fallback, honestly attributed
